package org.seasar.ymir.eclipse.maven.versioning;

import java.util.StringTokenizer;

/**
 * このクラスは以下のクラスを元に作りました。
 * http://svn.apache.org/repos/asf/maven/components/tags/maven-2.0.9/maven-artifact/src/main/java/org/apache/maven/artifact/versioning/DefaultArtifactVersion.java
 * 
 * @author yokota
 *
 */
        ...
        if (result == 0) {
            if (qualifier != null) {
                ...
                if (otherQualifier != null) {
                    if ((qualifier.length() > otherQualifier.length()) && qualifier.startsWith(otherQualifier)) {
                        // here, the longer one that otherwise match is considered older
                        result = -1;
                    } else if ((qualifier.length() < otherQualifier.length()) && otherQualifier.startsWith(qualifier)) {
                        // here, the longer one that otherwise match is considered older
                        result = 1;
                    } else {
