...
class PomTagEvaluator implements TagEvaluator {
    ...
    public String[] getSpecialTagPatternStrings() {
        return new String[] { "project", "build", "profiles", "repositories", "repository", "url", "dependencies", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    ...
    public String evaluate(TemplateContext context, String name, Attribute[] attributes, Element[] body) {
        ...
        if (ctx.isTopLevel()) {
            if ("project".equals(name)) { //$NON-NLS-1$
                ...
                return sb.toString();
            } else if ("build".equals(name) || "profiles".equals(name)) { //$NON-NLS-1$ //$NON-NLS-2$
                ctx.enter();
                try {
