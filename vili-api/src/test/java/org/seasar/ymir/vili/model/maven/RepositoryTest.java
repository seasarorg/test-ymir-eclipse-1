...
public class RepositoryTest extends TestCase {
    ...
    public void testToBean1() throws Exception {
        Repository actual = mapper.toBean(XMLParserFactory.newInstance().parse(
                new InputStreamReader(getClass().getClassLoader().getResourceAsStream(
                        getClass().getName().replace('.', '/').concat("_repository1.xml")), "UTF-8")).getRootElement(),
                Repository.class);
    ...
    public void testToBean2() throws Exception {
        Repository actual = mapper.toBean(XMLParserFactory.newInstance().parse(
                new InputStreamReader(getClass().getClassLoader().getResourceAsStream(
                        getClass().getName().replace('.', '/').concat("_repository2.xml")), "UTF-8")).getRootElement(),
                Repository.class);
    ...
    public void testToXML2() throws Exception {
        String expected = IOUtils.readString(new InputStreamReader(getClass().getClassLoader().getResourceAsStream(
                getClass().getName().replace('.', '/').concat("_repository2.xml")), "UTF-8"), true);
