...
/**
 * Copied from org.eclipse.jdt.internal.ui.text.correction.ChangeCorrectionProposal Implementation of a Java completion
 * proposal to be used for quick fix and quick assist proposals that invoke a {@link Change}. The proposal offers a
 * proposal information but no context information.
 * 
 * @since 3.2
 */
public abstract class ChangeCorrectionProposal implements IJavaCompletionProposal /*, ICommandAccess */{
   ...
   * @param change The change that is executed when the proposal is applied or <code>null</code> if the change will be
   *          created by implementors of {@link #createChange()}.
   * @param relevance The relevance of this proposal.
   * @param image The image that is displayed for this proposal or <code>null</code> if no image is desired.
   */
...
//			MavenPlugin.log("An exception occurred while applying the Quick Fix", e);
//		}
//	}
   ...
   * @param document The document of the editor currently active or <code>null</code> if no editor is visible.
...
//			return MessageFormat.format("{0} ({1} direct access)", new String[] { getName(), shortCutString });
//		}
    return getName();
  }
  ...
  /**
   * Creates the text change for this proposal. This method is only called once and only when no text change has been
   * passed in {@link #ChangeCorrectionProposal(String, Change, int, Image)}.
   * 
